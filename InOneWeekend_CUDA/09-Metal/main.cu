...
//kernel functions
__device__ vec3 rayColor(const ray& r, hittable **world, curandState *localRandState)
{
    ray cur_ray = r;
...
__global__ void renderInit(int max_x, int max_y, curandState *randState)
{
    ...
    curand_init(1864, pixelIndex, 0, &randState[pixelIndex]);
}
...
__global__ void render(vec3 *fb, int max_x, int max_y, int numSamples, camera **cam, hittable **world, curandState *randState)
{
    ...
    for (int s = 0; s < numSamples; s++)
    {
        ...
        color += rayColor(r, world, randState);
    }
